...
min_size = round(
  Int,
  sqrt(0.65 * StrideArrays.VectorizationBase.CACHE_SIZE[3] / sizeof(T))
)
max_size = round(
  Int,
  sqrt(32 * StrideArrays.VectorizationBase.CACHE_SIZE[3] / sizeof(T))
)
...
opt = Optim.optimize(
  ...
  init,
  ParticleSwarm(;
    lower = [0.001, 0.01, 0.3, 0.4],
    upper = [0.1, 2.0, 0.9, 0.99]
  ),
  Optim.Options(; iterations = 10^6, time_limit = days / 8)
);
